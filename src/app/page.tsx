...
export default function Home() {
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: "Yo, this is AshuBoi's ChatterBot! How can I help you today?",
    },
  ]);
  ...
  const callGetResponse = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Error:", data.error); // Handle server-side error
      }
    } catch (error) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between px-24 py-5">
      <h1 className="text-5xl font-sans">AshuBoi's ChatterBot</h1>
